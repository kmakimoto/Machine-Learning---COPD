# Code for Predicting COPD Severity with and without radiomics
# By: Kalysta Makimoto
...
# BEFORE RUNNING:
    # Update file location for the features (Line 40)
    # Upadate file location for the true labels (Line 45)
    # Update export files names (Line 212 and Line 213)
...
Train_FS_rf = RandomForestClassifier(n_estimators=100, random_state=0)
...
Train_FS_extratree = ExtraTreesClassifier(n_estimators=100, random_state=0)
...
Train_FS_gbdt = GradientBoostingClassifier(n_estimators=100, learning_rate=1.0)
...
Classifier_result_table = pd.DataFrame(columns=['classifiers', 'fs', 'best_param', 'best_auc_CV','No_fts', 'auc','f1', 'acc', 'precision', 'recall', 'features'])
...
# Define GridSearchCV parameters to test, match index with classifiers
parameters = [{'solver': ['lbfgs', 'liblinear','newton-cg', 'newtwon-cholesky'], 'C': [0.1, 1, 10]},
...
for names in FS_names:      # Investigate each feature selection method individually
    ...
    for n in n_features:        # Investigate different number of input features
        ...
        for x in range(len(classifiers)):
            ...
            print(model)
            param = parameters[x]        # Identify the GridSearchCV parameters to be investigated
            ...
            auc = roc_auc_score(Test_Label, yproba, multi_class='ovr')      # Calculates the AUC in the testing dataset
            f1 = f1_score(Test_Label, ypred, average='macro')       # Calculates the F1-score in the testing dataset
            acc = accuracy_score(Test_Label, ypred)     # Calculates the accuracy in the testing dataset
            precision = precision_score(Test_Label, ypred, average='macro')     # Calculates the precision in the testing dataset
            recall = recall_score(Test_Label, ypred, average='macro')       # Calculates the recall in the testing dataset
...
Classifier_result_table.to_excel(r'/Users/XX.xlsx')
